...
contract VestedWallet is Ownable {
    ...
    StentorCrowdsale contribution;
    StentorToken sgt;

    function VestedWallet(address _owner, address _contribution, address _sgt) {
        owner = _owner;
        contribution = StentorCrowdsale(_contribution);
        sgt = StentorToken(_sgt);
    }


    /// @notice The Dev (Owner) will call this method to extract the tokens
    function collectTokens() public onlyOwner {
        uint256 balance = sgt.balanceOf(address(this));
        uint256 total = collectedTokens.add(balance);

        uint256 finalizedTime = contribution.finalizedTime();

        require(contribution.isFinalized() && finalizedTime > 0 && getTime() > finalizedTime.add(months(6)));
        ...
        assert(sgt.transfer(owner, canExtract));
