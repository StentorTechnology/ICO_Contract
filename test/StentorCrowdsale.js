...
contract('StentorCrowdsale', async function (accounts) {
    ...
    it("Should allow the half of the tokens to vest one year after the crowdsale was finalized", async () => {
        ...
        const initialSupply = web3.toBigNumber(config.initialSupply);
        ...
        const calculatedTokens = web3.fromWei(initialSupply.mul(teamOwned).mul(.50).add(config.foundation.amount)).toNumber();
    ...
    it("Tokens should fully vest two years after the crowdsale has ended", async () => {
        ...
        const calculatedTokens = web3.fromWei(initialSupply.mul(teamOwned).add(config.foundation.amount)).toNumber();
        const realTokens = web3.fromWei(balance).toNumber();
        assert.equal(realTokens, calculatedTokens, "Tokens vested incorrectly");
    });
